...
# "coords": [lat, lon, elv] in downloader.py in EQTransformer/utils/
		station_json[station]["coords"] = [ float(coordinates[i + 1]), float(coordinates[i+2]), 100] # i set elevation to 100 because that's the average height in sumatra and i'm not given this information + it's not that important 
...
def pbs_writer(n_nodes, output_csv, job_name, env_name, project_code, no_execute = False):
	...
	output_pbs = os.path.join("/home/chenyu.li/ACEH_work/EQTransformer/eqt_detection_cy/pbs", job_name +".pbs")
	...
	with open(output_pbs, "w") as f:
		...
		f.write("module load sac\ncd $PBS_O_WORKDIR\nnprocs=`cat $PBS_NODEFILE|wc -l`\ninputfile=/home/chenyu.li/ACEH_work/EQTransformer/eqt_detection_cy/node_distributor.py\n")
		...
		if not no_execute:
			...
			f.write("/home/chenyu.li/ACEH_work/EQTransformer/eqt_detection_cy/pbs/runtime_scripts/{0}/${{PBS_ARRAY_INDEX}}.sh >& log/pbs/{0}/$PBS_JOBID.log 2>&1\n".format(job_name))
...
def encode_multirun(
	...
	if config:
        ...
        #PROJECT_ROOT=/home/chenyu.li/ACEH_work/EQTransformer/eqt_detection_cy
		if not model_path:
		...
		#station_json = os.path.join(PROJECT_ROOT,"station/json/all_stations.json")
