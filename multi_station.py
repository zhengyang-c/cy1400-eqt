...
def select_files(selector_file, start_date, end_date, y_jul = True, y_mon = False, all_csv_path = "station/all_aceh_sac.csv", output_file = ""):
	...
	_df = df[df["station"].isin(station_list)  & (df["dt"] >= start_date) & (df["dt"] <= end_date)]
...
def encode_multirun(
	...
	if make_sac_csv:
		select_files(station_file, start_day, end_day, output_file = sac_select, all_csv_path = make_sac_csv)
...
if __name__ == "__main__":
	...
	parser.add_argument("-make_sac_csv", help = "flag to make the job csv", default = False)
