...
#        |    |              \/            \/
  ...
  def event_plot(self, assoc_id, west = -104.5, east= -94, south = 33.5, north = 37.5, deltalon = 1.0, deltalat = 1.0):
    ...
    for index,sta in enumerate(stations):  # the comma is needed
      lon,lat = self.tt_stations_db_1D.query(Station1D.longitude,Station1D.latitude).filter(Station1D.sta==sta).first()
      lon_STA.append(lon)
      lat_STA.append(lat)
      sta_STA.append(sta)
    ...
    for single_phase in single_phases:
      lon,lat=self.tt_stations_db_1D.query(Station1D.longitude,Station1D.latitude).filter(Station1D.sta==single_phase.sta).first()
      #x,y = m(lon,lat)
      pick_time=single_phase.time
      travel_time=(pick_time-ot).total_seconds()
      if single_phase.phase=='P':
        tt,tt_uncert=self.distance_singlephase(single_phase.phase,travel_time)
        d_km=tt.d_km
    ...
    for i in range(len(radius_rainbow)):
      subax.scatter(radius_rainbow[i],s_p_rainbow[i],s=50,color=rainbow[i],zorder = 2)
  ...
  def distance_singlephase(self,phase,time):
    """ single_d_km(session,s_p)
    Look up the distance for an singel phase travel observation and return the closest value
    stored in the travel-time table
  
    Returns the closest tt_object,s_p_difference
    """    
    if phase=='P':
      min=self.tt_stations_db_1D.query(TTtable1D).filter(TTtable1D.p_tt<=time).order_by(TTtable1D.p_tt.desc()).first()
      max=self.tt_stations_db_1D.query(TTtable1D).filter(TTtable1D.p_tt>=time).order_by(TTtable1D.p_tt).first()
    ...
    if phase=='S':
      min=self.tt_stations_db_1D.query(TTtable1D).filter(TTtable1D.s_tt<=time).order_by(TTtable1D.s_tt.desc()).first()
      max=self.tt_stations_db_1D.query(TTtable1D).filter(TTtable1D.s_tt>=time).order_by(TTtable1D.s_tt).first()
