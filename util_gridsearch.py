...
def cell_fn(i,j,k, lb_corner, phase_info, station_info, tt, DX, DZ, TT_DX, TT_DZ, TT_NX, find_station_misfit = False, ref_mean = 0, ref_origin = 0, weights = None):
	...
	if find_station_misfit:
		...
		for _i in range(len(_station_list)):
			...
			# calculate travel times AND misfit
			station_misfit[_sta][_phase +"_tt"] = tt_cell[_i]
			station_misfit[_sta][_phase + "_misfit"] = (np.abs((min_origin_time + datetime.timedelta(seconds = guess_ot[_i] - ref_mean) - datetime.datetime.fromtimestamp(ref_origin)).total_seconds())) 
	...
	if get_grid:
		...
		if do_rotate:
			...
			for _station in _station_list:
				...
				rotater(_station, args["pid"], args["event_folder"], save = True, output_folder = os.path.join(args["output_folder"], args["pid"]+"_c"),  best_baz = _baz)
